...
# OLS4 가 ontology=uberon 필터에도 타 온톨로지(예: PBMC→CL:2000001) CURIE 를 반환하는 경우가
# 있어, 결과 CURIE 의 prefix 가 요청 온톨로지와 일치하는지 강제한다. 미적용 시 tissue 슬롯에
# CL 코드가 새어드는 오태깅 발생(2026-06-17 sol4 shadow 에서 7% 검출). map_sra_to_curie.py 의
# 동일 가드와 정합.
_ONTO_EXPECTED_PREFIX: dict[str, str] = {
    "mondo": "MONDO:",
    "uberon": "UBERON:",
    "cl": "CL:",
    "efo": "EFO:",
...
class OntologyMapper:
    ...
    async def lookup(self, text: str, ontology: Ontology) -> Match | None:
        ...
        expected_prefix = _ONTO_EXPECTED_PREFIX.get(ontology)
        for doc in docs:
            ...
            if not curie:
                continue
            # prefix 가드: 요청 온톨로지와 다른 CURIE(예: uberon 조회에 CL 반환) 거름.
            if expected_prefix and not str(curie).upper().startswith(expected_prefix):
                continue
